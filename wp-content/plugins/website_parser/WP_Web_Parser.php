...
class WP_Web_Parser {
	...
	public static function init() {
		$wpwp_options = get_option('wpwp_options');
		if($wpwp_options['sc_posts'] == 1)
			add_shortcode('wpwp', array( 'WP_Web_Parser', 'shortcode' ));
		if($wpwp_options['sc_widgets'] == 1)
			add_filter('widget_text', 'do_shortcode');
	}

	public static function view( $name, $vars = array() ) {
		load_plugin_textdomain( 'wp-web-scraper' );
		$file = WPP_PLUGIN_DIR . 'views/'. $name . '.php';
		if(!empty($vars))
	...
	public static function plugin_activate() {
		$default_wpwp_options = array(
			'sc_posts' => 1,
			'sc_widgets' => 1,
			'tt' => 1,
			'on_error' => 'error_show',
			'useragent' => 'WPWP bot ('.get_bloginfo('url').')',
			'timeout' => 2,
			'cache' => 60
		);
		add_option( 'wpwp_options', $default_wpwp_options );
	}
	...
	public static function shortcode($atts){
		...
		return WP_Web_Parser::get_content($args['url'], $args['query'], $args);
	}
	...
	public static function get_content($url, $query = '', $args = array()) {
		...
		require_once WPP_PLUGIN_DIR.'website_parser.php';
		...
		$default_args = WP_Web_Parser::get_default_args();

		WP_Web_Parser::$url = $url;
		WP_Web_Parser::$query = $query;
		WP_Web_Parser::$args = wp_parse_args( $args, $default_args );
		WP_Web_Parser::$error = null;

		$response = WP_Web_Parser::remote_request($url, WP_Web_Scraper::$args);

		if( !is_wp_error( $response ) ) {

			$wpwp_parser = new WP_Web_Parser( $response['body'], WP_Web_Parser::$args['charset'] );
			WP_Web_Parser::$xcache = $response['headers']['X-WPWS-Cache-Control'];
			...
			if($query === ''){
				...
				if(WP_Web_Parser::$args['query_type'] === 'cssselector')
					$wpwp_parser->parse_selector( $query );
				if(WP_Web_Parser::$args['query_type'] === 'xpath')
					$wpwp_parser->parse_xpath( $query );
				if(WP_Web_Parser::$args['query_type'] === 'regex')
					$wpwp_parser->parse_regex( $query );

				if($wpwp_parser->error !== null){
					WP_Web_Parser::$error = "Error parsing: ".$wpwp_parser->error;
				} elseif(version_compare(PHP_VERSION, '5.3.3', '<')){
					WP_Web_Parser::$error = "Error parsing: PHP version 5.3.3 or greater is required for parsing";
				} else {
					$content = $wpwp_parser->result;
				}
			}

		} else {

			WP_Web_Parser::$error = "Error fetching: ".$response->get_error_message();
		}
		...
		if ( WP_Web_Parser::$args['debug'] == 1 ){
			$ob_header = PHP_EOL.
			             '<!--' . PHP_EOL .
			             ' Start of web scrap (created by wp-web-scraper)' . PHP_EOL .
			             ' Source URL: ' . WP_Web_Parser::$url . PHP_EOL .
			             ' Query: '. WP_Web_Parser::$query . ' (' . WP_Web_Parser::$args['query_type'] . ')' . PHP_EOL .
			             ' Other options: ' . print_r(WP_Web_Parser::$args, true) . '-->' . PHP_EOL;
			$ob_footer =  PHP_EOL.
			              '<!--' . PHP_EOL .
			              ' End of web scrap' . PHP_EOL .
			              ' WPWS Cache Control: ' . WP_Web_Parser::$xcache . PHP_EOL .
			              ' Computing time: ' . round(microtime(true) - $mt_start, 4) . ' seconds' . PHP_EOL .
			              '-->' . PHP_EOL;
		}

		if (WP_Web_Parser::$error === null) {

			$ob_body = WP_Web_Parser::filter_content($content, WP_Web_Parser::$args);

		} else {

			if ( WP_Web_Parser::$args['on_error'] === 'error_hide' ){
				$ob_body = '';
			} elseif ( WP_Web_Parser::$args['on_error'] === 'error_show' ){
				$ob_body = WP_Web_Parser::$error;
			} elseif ( !empty( WP_Web_Parser::$args['on_error'] ) ){
				$ob_body = WP_Web_Parser::$args['on_error'];
			}
